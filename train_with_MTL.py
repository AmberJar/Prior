...
from helper.loss_helper import LSCE_GDLoss_GLW, LabelSmoothSoftmaxCE
...
def main(rank, world_size, config):
    ...
    loss_fn = LSCE_GDLoss_GLW(ignore_index=255)
            ...
            if cfg.trainer.cutmix:
                if beta > 0 and r < cutmix_prob:
                    ...
                    with autocast(enabled=cfg.trainer.fp16):
                        ...
                        # output = model(images)
                        # compute output
                        loss1 = loss_fn(output, target_a) * lam
                        loss2 = loss_fn(output, target_b) * (1. - lam)
                        loss = [x + y for x, y in zip(loss1, loss2)]

                        loss1 = loss_fn(out_aux, target_a) * lam
                        loss2 = loss_fn(out_aux, target_b) * (1. - lam)
